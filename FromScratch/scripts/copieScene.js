...
var createScene = function() {
    ...
    $("#titrboutons").append('<button id="clickbox" class="btn"  > Ajouter un cube </button>'); //cube
  ...
  //
  //       // X AXIS
  //         var axisX = BABYLON.Mesh.CreateLines("axisX", [BABYLON.Vector3.Zero(),BABYLON.Vector3.Zero(), new BABYLON.Vector3(size, 0, 0)], scene);
  ...
  //       var pickInfo = scene.pick(scene.pointerX, scene.pointerY);
  //       if (pickInfo.hit && pickInfo.pickedMesh.name != "ground1") {
  ...
  //                 if(scene.meshes[i].name == "axisX" || scene.meshes[i].name == "axisY" || scene.meshes[i].name == "axisZ"){
  //                   scene.meshes[i].name.isVisible = true;
  //                 }
  //               }
  ...
  //
	...
	// 	for(var i = 0; i < currentMesh.childNodes.length; i++)
	// 		currentMesh.childNodes[i].isVisible = true;
  //
	// 	// Remove axis for the previous mesh
	// 	if(lastMesh != null){
	// 		if(lastMesh.childNodes.length > 0)
	// 			for(var i = 0; i < lastMesh.childNodes.length; i++)
	// 				lastMesh.childNodes[i].isVisible = false;
	// 	}
	// }
